...
const Navbar = () => {
  ...
  // Handler for CTA button: open Clerk modal (sign in or sign up)
  const handleCTA = () => {
    // If signed in, go to dashboard; else open Clerk modal
    if (isSignedIn) {
      window.location.assign('/dashboard');
    } else {
      // Use Clerk's SignInButton to open modal
    ...
    <nav className="bg-slate-900/95 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* CTA Button */}
          <div className="hidden md:block">
            {/* Hidden button for Clerk modal trigger */}
            <SignInButton mode="modal">
              <Button id="clerk-landing-cta" style={{ display: 'none' }} />
            </SignInButton>
            <Button className="bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-2 font-semibold" onClick={handleCTA}>
              Get Started
            </Button>
          </div>
          ...
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-800 border-t border-slate-700">
              ...
              <div className="px-3 py-2">
                {/* Hidden button for Clerk modal trigger */}
                <SignInButton mode="modal">
                  <Button id="clerk-landing-cta-mobile" style={{ display: 'none' }} />
                </SignInButton>
                <Button className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-semibold" onClick={handleCTA}>
                  Get Started
                </Button>
              </div>
            </div>
